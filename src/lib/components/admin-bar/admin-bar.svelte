<script lang="ts">
	...
	// Apply the `theme` prop only if the user hasn't already picked one
	// (no localStorage value yet). After that, the singleton in
	// `theme.svelte.ts` is the source of truth.
	untrack(() => {
		if (!browser) return;
		if (localStorage.getItem(THEME_STORAGE_KEY) !== null) return;
		if (theme === adminBarTheme.pref) return;
